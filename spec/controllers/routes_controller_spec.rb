...
RSpec.describe RoutesController, type: :controller do
  ...
  let(:invalid_attributes) {
    FactoryGirl.attributes_for(:route, passphrase: nil, source: nil)
  }
  ...
  describe 'GET #show' do
    it 'returns a success response' do
      ...
      get :show, params: {passphrase: route.passphrase, source: route.source}, session: valid_session
    ...
    it 'returns the expected route object' do
      route = Route.create! valid_attributes
      get :show, params: {passphrase: route.passphrase, source: route.source}, session: valid_session
      expect(JSON.parse(response.body)).to have_key('passphrase')
      expect(JSON.parse(response.body)['passphrase']).to eq(route.passphrase)
      expect(JSON.parse(response.body)).to have_key('source')
      expect(JSON.parse(response.body)['source']).to eq(route.source)
    end
  end
